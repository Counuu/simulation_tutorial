...
for i = 1:sim_par.n_part %For now one sim, later with different parameter settings
    ...
    % ==== Initialise values as no prior experience exists ================
    ...
    % Initialise P_Go to save Action Probabilities for plotting 
    P_GO = [];
    ...
    for t = 1:sim_par.n_trials
        s = stim_pres(t); % currently presented stimulus (condition) 
                ...
                % ==== Chose an Action ====================================
                % Action Weight for go and no-go, for each possible action
                for a = 1:2
                    ActionWeight_go(s,a) = Q(s,a) + sim_par.zeta ;
                    ActionWeight_nogo(s,a) =  Q(s,a) ;
                end
                
                % Transform Weight in Action Probability (0-1)(softmax function)
                softmax_prob = softmax(ActionWeight_go); 
                    
                % Action Probability for Go Action 
                ActionProb = softmax_prob * (1 - sim_par.xi) + (sim_par.xi/2);
                    
                % Keep track off all go probabilities (P(Go)) for plotting
                P_GO = [P_GO;ActionProb(s,2)]; 
              
                % Choose Action with the highest Action probability 
                [r,c] = find(ActionProb == max(ActionProb(:,:))); 
                
                % Action Choice 0 = No-Go, 1 = Go 
                if length(c) == 1    
                    if c == 1 
                ...
                else % if equally high, random action decision
                    ActionChoice(t) = binornd(1, 0.5); 
                end 
...
hold on 
plot(P_GO(stim_pres == 2))
hold on 
plot(P_GO(stim_pres == 3))
hold on 
